...
import { ChangeDetector, DEFAULT_ROUTES_FILE } from "./types";
...
const makeAddDebugEndpointHandler = (changeDetector: ChangeDetector) => {
  const addDebugYaml = (
    ...
    if (req.body.fileName) {
      if (
        incrementSameNameEndpoint(
          addDebugYaml.target.routes.endpoints,
          req.body.fileName
        )
      ) {
        // already exists
      ...
      const endpoints = makeEndpointsFromYaml(
        raw.toString(),
        req.body.fileName
      );
...
const makeDeleteDebugEndpointHandler = (changeDetector: ChangeDetector) => {
  const removeDebugYaml = (
    ...
    if (req.body.fileName) {
      if (
        !decrementSameNameEndpoint(
          removeDebugYaml.target.routes.endpoints,
          req.body.fileName
        )
      ) {
        res.status(200).send();
...
const addEndpointsHandler = (changeDetector: ChangeDetector) => {
  const addYaml = (
    ...
    if (
      incrementSameNameEndpoint(
        addYaml.target.routes.endpoints,
        req.params["name"]
      )
    ) {
      // already exists
...
const getStateHandler = (req: express.Request, res: express.Response) => {
  res.set("content-type", "application/json");
...
export const controlRouter = (
  ...
  debugRouter.post(
    "/debug/endpoints",
    makeAddDebugEndpointHandler(changeDetector)
  );
  debugRouter.delete(
    "/debug/endpoints",
    makeDeleteDebugEndpointHandler(changeDetector)
  );
