...
const tryReadConfig = (
  fileName: string = defaultConfigFile
...
commander
  ...
  .usage("[options]")
  .option(
    "-i --input <fileName | dirName>",
    "input file or directory name(required)"
  )
  .option(
    "-o --output <fileName>",
    "output file name for routes.json(default: 'routes.json'"
  )
  ...
  .option(
    "-c --suppress-content-length",
    "set suppressContentLength flag(omitted)"
  )
...
const yamlPat2 =
  typeof config.all === "string" ? new RegExp(config.all) : config.all;
...
try {
  ...
  for (const target of targets) {
    try {
      ...
      const endpoint = makeEndpointsFromYaml(
        content,
        target,
        requiredOnly,
        config
