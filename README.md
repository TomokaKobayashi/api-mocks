...
# 新機能
## ver.0.1.38
- 受信可能なリクエストサイズの指定ができるようになりました。
## ver.0.1.39
- httpsを暫定サポート
- マルチパートリクエスト時にアップロードファイルを自動消去するかどうかを選択できるようになりました。
...
| -t | なし | httpsを有効かします。 |
| -w | ドメイン名 | httpsで利用するドメイン名を指定します。 | localhost |
| -m | 最大受信サイズ | リクエストを受信する最大サイズを指定します。"100kb"や"10mb"のように指定します。 | 10mb |
| -k | なし | アップロード一時ファイルを保持します。 |
...
### 9.httpsのサポート
ver.0.1.39から暫定的にhttpsを使えるようになりました。  
```-t```オプションを指定することでドメイン名```localhost```でhttps通信を行うことができます。  
```-w```オプションでドメイン名を指定することも可能です。
...
| uploadPath | string | - | アップロード一時ファイルを格納するディレクトリを指定します。 |
| keepUploadFile | boolean | - | アップロード一時ファイルを保持するかどうかを指定します。 |
