...
const createRequestModifier = (validatorArgs) => {
    ...
    return (req) => {
        ...
        if (modifierList.cookie)
            modifierList.path.forEach((modify) => { modify(req.cookies); });
    };
};
/// making a endpoint handler
const createHnadler = (baseDir, endpoint, defaultHeaders) => {
        ...
        for (const endpoint of routes.endpoints) {
            switch (endpoint.method) {
                case "GET":
                    ...
                    mockRouter.get(endpoint.pattern, createHnadler(baseDir, endpoint, routes.defaultHeaders));
                    break;
                case "POST":
                    console.log(`POST   : ${endpoint.pattern}`);
                    mockRouter.post(endpoint.pattern, createHnadler(baseDir, endpoint, routes.defaultHeaders));
                    break;
                case "PUT":
                    console.log(`PUT    : ${endpoint.pattern}`);
                    mockRouter.put(endpoint.pattern, createHnadler(baseDir, endpoint, routes.defaultHeaders));
                    break;
                case "PATCH":
                    console.log(`PATCH  : ${endpoint.pattern}`);
                    mockRouter.patch(endpoint.pattern, createHnadler(baseDir, endpoint, routes.defaultHeaders));
                    break;
                case "DELETE":
                    console.log(`DELETE : ${endpoint.pattern}`);
                    mockRouter.delete(endpoint.pattern, createHnadler(baseDir, endpoint, routes.defaultHeaders));
                    break;
                default:
